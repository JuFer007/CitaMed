...
public class CitaService {
    ...
    public String saveCita(CitaDTO dto) {
        Paciente paciente = pacienteRepository.findById(dto.getPacienteId()).orElse(null);
        if (paciente == null)
            return "Paciente no encontrado con ID: " + dto.getPacienteId();

        Medico medico = medicoRepository.findById(dto.getMedicoId()).orElse(null);
        if (medico == null)
            return "Médico no encontrado con ID: " + dto.getMedicoId();

        if (!medico.isActivo())
            return "El médico no está activo";

        Consultorio consultorio = consultorioRepository.findById(dto.getConsultorioId()).orElse(null);
        if (consultorio == null)
            return "Consultorio no encontrado con ID: " + dto.getConsultorioId();

        if (!consultorio.isDisponible())
            return "El consultorio no está disponible";

        boolean medicoOcupado = citaRepository.existsByMedicoIdAndFechaHoraAndEstadoNot(dto.getMedicoId(), dto.getFechaHora(), EstadoCita.CANCELADA);

        if (medicoOcupado)
            return "El médico ya tiene una cita en ese horario";

        boolean pacienteOcupado = citaRepository.existsByPacienteIdAndFechaHoraAndEstadoNot(dto.getPacienteId(), dto.getFechaHora(), EstadoCita.CANCELADA);
