...
public interface CitaRepository extends JpaRepository<Cita, Long> {
    ...
    @Query(value = "SELECT DATE_FORMAT(citas.fecha_hora,'%h:%i %p') AS hora, CONCAT(pacientes.nombre,' ', " +
    "pacientes.apellido_paterno,' ', pacientes.apellido_materno) AS paciente, CONCAT(especialidades.nombre,' - ', " +
    "medicos.nombre,' ', medicos.apellido_paterno) AS detalle FROM citas INNER JOIN medicos ON citas.medico_id = medicos.id" +
    "INNER JOIN pacientes ON citas.paciente_id = pacientes.id INNER JOIN especialidades" +
    "ON medicos.especialidad_id = especialidades.id WHERE YEAR(citas.fecha_hora) = :anio " +
    "AND MONTH(citas.fecha_hora) = :mes  AND DAY(citas.fecha_hora) = :dia ORDER BY citas.fecha_hora", nativeQuery = true)
